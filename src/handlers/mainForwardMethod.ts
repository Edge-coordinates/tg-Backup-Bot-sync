...
export function registerForwardMethod(bot: Bot) {
  bot.command(
    "backup",
    withAuth(async (ctx) => {
      ...
      await mainForwardMethod(ctx, startLink, endLink);
    })
  );
}
...
export async function mainForwardMethod(
  ctx: Context,
  startLink: string,
  endLink: string
) {
...
async function handleMessageByType(ctx: Context, msg: Message) {
  ...
  if (msg.photo || msg.video) {
    ...
    //   let tmsg = buildTextMessageObject(replyResult.combinedText + source);
      let tmsg = buildTextMessageObject(replyResult.combinedText);
    ...
    //   const textMsg = buildFormattedTextMessage(msg.text, msg.entities, source);
      ...
      try {
        await ctx.api.sendMessage(targetChannelID, (msg as any).text, {
          // parse_mode: "HTML",
